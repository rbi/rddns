...
async fn update_via_http(ddns_entry: &DdnsEntryHttp, resolved_url: &String) -> Result<(), String>{
    ...
    let https_connector = HttpsConnectorBuilder::new().with_native_roots().https_or_http().enable_http1().build();
    ...
    let result = client.request(request.body(Body::empty()).unwrap()).await.map_err(|err| err.to_string())?;
...
async fn update_file(file: String, resolved_content: String) -> Result<(), String> {
    write(file, resolved_content).await.map_err(|err| err.to_string())
}
