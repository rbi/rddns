...
pub fn resolve_derived(config: &IpAddressDerived, address_actual: &HashMap<String, IpAddr>) -> Option<IpAddr> {
    resolve_derived_ip(address_actual.get(&config.subnet_entry), address_actual.get(&config.host_entry), config.subnet_bits)
}

fn resolve_derived_ip(net_address: Option<&IpAddr>, host_address: Option<&IpAddr>, subnet_bits: u8) -> Option<IpAddr> {
...
mod tests {
    ...
    fn some_addresses() -> HashMap<String, IpAddr> {
        ...
        address_values.insert("net_ip2".to_string(), "2001:DB8:a2f3:aaaa::29".parse().unwrap());
        address_values.insert("host_ip2".to_string(), "::4bcf:78ff:feac:8bd9".parse().unwrap());
        address_values
    }
    ...
    fn resolve_handles_derived_addresses() {
        ...
        assert_eq!(Some("203.0.113.42".parse().unwrap()),
                   resolve_derived(&IpAddressDerived {
                       subnet_bits: 24,
                       subnet_entry: "net_ip1".to_string(),
                       host_entry: "host_ip1".to_string(),
                   }, &address_values));

        assert_eq!(Some("2001:db8:a2f3:aa00:4bcf:78ff:feac:8bd9".parse().unwrap()),
                   resolve_derived(&IpAddressDerived {
                       subnet_bits: 56,
    ...
    fn resolve_handles_derived_addresses_with_maximal_possible_subnet() {
        ...
        assert_eq!(Some("203.0.113.25".parse().unwrap()),
                   resolve_derived(&IpAddressDerived {
                       subnet_bits: 32,
                       subnet_entry: "net_ip1".to_string(),
                       host_entry: "host_ip1".to_string(),
        ...
        assert_eq!(Some("2001:db8:a2f3:aaaa::29".parse().unwrap()),
                   resolve_derived(&IpAddressDerived {
                       subnet_bits: 128,
                       subnet_entry: "net_ip2".to_string(),
                       host_entry: "host_ip2".to_string(),
    ...
    fn resolve_handles_derived_addresses_with_minimal_possible_subnet() {
        ...
        assert_eq!(Some("0.0.0.42".parse().unwrap()),
                   resolve_derived(&IpAddressDerived {
                       subnet_bits: 0,
                       subnet_entry: "net_ip1".to_string(),
                       host_entry: "host_ip1".to_string(),
        ...
        assert_eq!(Some("::4bcf:78ff:feac:8bd9".parse().unwrap()),
                   resolve_derived(&IpAddressDerived {
                       subnet_bits: 0,
                       subnet_entry: "net_ip2".to_string(),
                       host_entry: "host_ip2".to_string(),
